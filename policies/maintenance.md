...
개념 문서는 학습자가 이해·적용·판단하도록 돕는 기술 설명문으로 작성한다. 한국어 설명은 차분한 합니다체를 사용하고, 한 문장에 새로운 용어를 몰아넣지 않는다. 첫 등장에 쉬운 설명과 원어를 연결하고 이후 같은 표현을 유지한다. 목록 앞뒤에는 원인·결과와 조건을 설명하는 문장을 둔다.

- 시작: 독자가 겪을 상황, 학습 목표와 선수 지식을 제시한다. 선수 문서는 실제 존재하는 원본으로 연결한다.
- 101: 개념·필요성·용어를 설명한다. 이미 아는 사람을 위한 압축 정의만으로 시작하지 않는다.
- 201: 가정 → 적용 또는 계산 → 기대 결과와 해석을 제공한다. 설계 연습을 실제 실행한 실습으로 표시하지 않는다.
- 301: 조건·대안·운영 확인과 판단 근거를 연결한다. 이해 확인 질문에는 해설이나 확인 기준을 제공한다.
- 범위: 모든 분야에 완성된 강좌가 있다고 약속하지 않는다. 번호를 위해 원본을 세 곳으로 복제하거나 경로·concept_id를 바꾸지 않는다. 번호는 본문의 학습 단계이며 `learning_state` 또는 OKF 표준 난이도 필드가 아니다.
- 종류: 용어집은 짧은 정의·예제·혼동 방지를 유지한다. 체크리스트와 런북은 실행 조건과 완료 기준을 명확히 한다. ADR·실험·실패의 당시 본문과 증거는 보존한다.
- 근거: 검증자·세션 설명의 반복은 메타데이터와 log로 정리한다. 주장별 출처, 적용 조건, 미검증 범위를 지우지 않는다. 예제 계산과 실제 측정값을 구분한다.

이 편집은 사용자가 승인한 학습 설명 개선에 적용한다. 정기 Verify/Refresh는 아래의 사실 변경 범위를 유지하며 문체 개선을 이유로 전수 재작성을 자동 실행하지 않는다. 설명·예제·학습 목표의 의미 변경은 `generated`와 log에 기록하고 영어의 목표·조건·해설을 함께 대조한다. 재검증하지 못한 내용은 과거 verified 사건을 새 내용의 검증처럼 사용하지 않는다.
