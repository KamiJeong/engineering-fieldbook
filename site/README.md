...
넓은 홈 화면은 소개와 학습 경로를 나란히 배치하고, 작은 화면에서는 읽는 순서대로 쌓습니다. 주제 목록의 설명과 문서 수는 기존 공개 데이터에서 가져옵니다. 문서 본문은 한글 단어 단위 줄바꿈을 우선하며, 현재 문서의 배경 강조와 선택한 학습 경로의 링크 강조를 구분합니다.
...
브라우저 테스트는 SPA fallback 없는 Bun 정적 서버에서 전체 공개 문서와 내부 링크의 HTTP 응답/앵커, 대표 문서의 JavaScript 비활성 읽기, 검색·언어 전환·404·코드 복사·Mermaid를 확인한다. 375/768/1440px와 라이트/다크 홈·문서 캡처는 `artifacts/ui-review/verification`에 생성한다. Chromium 검증이며 Safari/Firefox, 실제 GitHub Pages 도메인과 원격 Actions 실행은 별도 확인이 필요하다.
